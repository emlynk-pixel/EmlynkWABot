...
| `created_date` | TIMESTAMP | NOT NULL | Creation |
| `expires_date` | TIMESTAMP | Nullable | Cleanup deadline |
...
```mermaid
flowchart TD
    A[Police Report Slip] --> B[OCR]
    B --> C[Extract Submitted Date]
    C --> D{Date Valid}
    D -->|No| E[Manual Review]
    D -->|Yes| F[Calculate Submitted Date Plus 21 Days]
    F --> G[Calculate Remaining Days]
    G --> H{Status}
    H -->|More Than 7 Days| I[Pending]
    H -->|1 to 7 Days| J[Due Soon]
    H -->|Zero Days| K[Due Today]
    H -->|Less Than Zero| L[Overdue]
    I --> M[Admin Dashboard]
    J --> M
    K --> M
    L --> M
...
temporary_data.expires_date < CURRENT_TIMESTAMP
...
flowchart TD
    ...
    Temporary --> TemporaryIdentifier[identifier]
    TemporaryIdentifier --> TemporaryDocuments[documents]

    Undefined --> MobileFolder[mobile_number]
    MobileFolder --> Uncleared[uncleared-docs]
```
...
The webhook should acknowledge receipt quickly and process the document asynchronously.
...
|---|---|---|---|
| WhatsApp API failure | API error/timeout | Retry | Exponential backoff |
| File download failure | Download error | Retry | Retry with limit |
| Unsupported file | MIME/extension validation | Undefined | Admin review |
| Corrupt file | File parser failure | Undefined | Admin review |
| OCR failure | AI/OCR error | Retry | Manual review |
| Low OCR confidence | Confidence threshold | Do not overwrite | Manual review |
| Passport ID missing | Extraction result | Undefined/temporary | Manual review |
| Passport mismatch | Identity comparison | Conflict | Admin review |
| WhatsApp mismatch | Identity comparison | Conflict | Admin review |
| Duplicate document | Hash/message check | Idempotent response | No duplicate processing |
| Storage failure | Storage exception | Keep retryable state | Retry |
| DB failure | Transaction exception | Do not finalize | Retry |
| Finalization failure | Transaction/storage check | Keep temporary | Retry |
| Reminder failure | Scheduler error | Recalculate on next run | Monitoring |
...
CREATE INDEX idx_temporary_expiry
    ON temporary_data (expires_date);
```
